...
<head>
	...
	<body>
		...
		<p class="descriptiontext">Number of Fitness Contributors:</p>
	...
	<?php
		...
		$query = "SELECT (SELECT COUNT(`keepfitid`) FROM `keepfit` WHERE `keepfitid` = 'KFE') AS Exercise_Experts, (SELECT COUNT(`keepfitid`) FROM `keepfit` WHERE `keepfitid` = 'KFD') AS Diet_Experts, (SELECT COUNT(`keepfitid`) FROM `keepfit`) AS Total";
